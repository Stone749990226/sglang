...
class MegaAttentionRuntime:
    ...
    def should_use_mega(self, forward_batch: ForwardBatch, layer) -> bool:
        ...
        if num_row_tiles > self.ws.max_num_row_tiles or len(
            forward_batch.extend_seq_lens_cpu
        ) > self.ws.max_num_batch:
            return False
        return True
    ...
    def _build_workspace(self, layer, forward_batch):
        ...
        if T <= 0:
            raise RuntimeError("chunked_prefill_size 未启用,mega 需要 chunked prefill 容量上界")
        ...
        try:
            ...
            logger.warning("megaAttention choose_launch_config failed, using defaults: %r", e)
        ...
        ws.compile(w_fa=w_fa, w_oproj=w_oproj, w_ar=w_ar, softmax_scale=float(D) ** -0.5)
        self.ws = ws
        logger.info(
            ...
            tp_size, rank, H_local, H_kv_local, D, hidden, q_per_kv,
            max_num_row_tiles, max_num_batch, max_num_pages, max_num_pages_per_seq,
            w_fa, w_oproj, w_ar, sg,
        )
    ...
    def get_or_build_prefill_meta(self, forward_batch: ForwardBatch) -> _PreparedMeta:
        ...
        page_table = (
            req_to_token[forward_batch.req_pool_indices][:, : max_pages * 128 : 128] // 128
        ).to(torch.int32)
        cache_seqlens = seq_lens.to(torch.int32)
        seqlens_q = forward_batch.extend_seq_lens_cpu  # host List[int],build_row_desc 用
